...
class FallDetection:
    def __init__(self, model_path, target_eps=1.2, scale_factor=0.6, one_person_threshold=0.8, min_frame_ratio=0.1):
    ...
    #프레임별 데이터 클러스터링
    def cluster_data(self, file_path):
    ...
    def run_model(self, cluster_dataframes):
        for cluster_id, cluster_df in cluster_dataframes.items():
            ...
            # print(f"Predictions for Cluster {cluster_id}: {predictions}")

    def run_pipeline(self, file_path):
        clustered_data = self.cluster_data(file_path)
...
if __name__ == "__main__":
    # file_path = r"src\AI\cloud_sit_close.csv"    # sit_close
    file_path = r"src\AI\cloud_walk1.csv"    # walk1
    model_path = r"src\AI\final_model.h5"
    pipeline = FallDetection(model_path)
